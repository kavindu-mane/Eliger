...
const ViewBooking = lazy(() => import("./Modals/ViewBooking"));
const Paginations = lazy(() => import("../Common/Paginations"));

// create sweet alert object
const Alert = withReactContent(Swal);

const ViewRentOutBooking = () => {
  ...
  const fetch = useCallback(async () => {
    ...
    await axios
      .post("/get_rentout_booking", formData)
      .then((response) => {
          ...
          return (
            ...
            >
              <p className="flex w-full truncate bg-slate-100 px-4 py-3 capitalize group-hover:bg-gray-200 dark:bg-slate-900 group-hover:dark:bg-gray-800">
                <span className="block md:hidden">Plate Number :&ensp;</span>
                ...
                <span
                  className={
                    ["rejected", "canceled"].includes(data?.Booking_Status)
                      ? "text-red-500"
                      : ["approved", "finished", "driving"].includes(
                          data?.Booking_Status
                        )
                      ? "text-green-500"
                      : "text-orange-400"
...
export default ViewRentOutBooking;
